...
age_plymouth<-age_data |> filter(Lower.layer.Super.Output.Areas %in% plymouth_lsoas)

age_sex_plymouth<-age_sex_data |> filter(Lower.layer.Super.Output.Areas %in% plymouth_lsoas)

disability_plymouth<-disability_data |> filter(Lower.layer.Super.Output.Areas %in% plymouth_lsoas)

disability_sex_plymouth<-disability_sex_data |> filter(Lower.layer.Super.Output.Areas %in% plymouth_lsoas)

education_plymouth<-education_data |> filter(Lower.layer.Super.Output.Areas %in% plymouth_lsoas)

education_sex_plymouth<-education_sex_data |> filter(Lower.layer.Super.Output.Areas %in% plymouth_lsoas)

ethnicity_plymouth<-ethnicity_data |> filter(Lower.layer.Super.Output.Areas %in% plymouth_lsoas)

ethnicity_sex_plymouth<-ethnicity_sex_data |> filter(Lower.layer.Super.Output.Areas %in% plymouth_lsoas)


## Save data for the explainer packs


write.csv(age_plymouth,"../Data for Explainer Pack/plymouth_age.csv",row.names = FALSE)
write.csv(age_sex_plymouth,"../Data for Explainer Pack/plymouth_age_sex.csv",row.names = FALSE)

write.csv(disability_plymouth,"../Data for Explainer Pack/plymouth_disability.csv",row.names = FALSE)
write.csv(disability_sex_plymouth,"../Data for Explainer Pack/plymouth_disability_sex.csv",row.names = FALSE)

write.csv(education_plymouth,"../Data for Explainer Pack/plymouth_education.csv",row.names = FALSE)
write.csv(education_sex_plymouth,"../Data for Explainer Pack/plymouth_education_sex.csv",row.names = FALSE)

write.csv(ethnicity_plymouth,"../Data for Explainer Pack/plymouth_ethnicity.csv",row.names = FALSE)
write.csv(ethnicity_sex_plymouth,"../Data for Explainer Pack/plymouth_ethnicity_sex.csv",row.names = FALSE)

## Age


age_plymouth_total<-age_plymouth |> group_by(Age..6.categories.) |> summarise(total=sum(Observation)) |> 
  mutate(proportion=(total/6749)*100)


age_sex_plymouth_total<-age_sex_plymouth |> group_by(Sex..2.categories.,Age..6.categories.) |> 
  summarise(total=sum(Observation))

ggplot(age_plymouth_total,aes(x=Age..6.categories.,y=total))+geom_bar(stat = "identity")

ggplot(age_sex_plymouth_total,aes(x=Age..6.categories.,y=total,fill=Sex..2.categories.))+
  geom_bar(stat = "identity",position = position_dodge(width = 0.9))

## Order by percentage
age_plymouth_total$Age..6.categories_f <- factor(
  age_plymouth_total$Age..6.categories.,
  levels = age_plymouth_total$Age..6.categories.[order(age_plymouth_total$proportion)]
)

ggplot(age_plymouth_total, aes(x = proportion, y = Age..6.categories.)) +
  geom_segment(aes(x = 0, xend = proportion,
                   y = Age..6.categories., yend = Age..6.categories.),
               colour = "grey70",
               linewidth = 1) +
...
## Order by percentage
age_sex_plymouth_total$Age..6.categories. <- factor(
  age_sex_plymouth_total$Age..6.categories.,
  levels = age_plymouth_total$Age..6.categories.[order(age_plymouth_total$proportion)]
)

ggplot(age_sex_plymouth_total, aes(x = total, y = Age..6.categories.,colour = Sex..2.categories.)) +
  geom_segment(aes(x = 0, xend = total,
                   y = Age..6.categories., yend = Age..6.categories.),
               colour = "grey70",
               linewidth = 1) +
...
age_sex_plymouth_total$Age..6.categories_f <- factor(
  age_sex_plymouth_total$Age..6.categories.,
  levels = age_plymouth_total$Age..6.categories.[order(age_plymouth_total$Age..6.categories.)]
)
...
ethnicity_plymouth_total<-ethnicity_plymouth |> group_by(Ethnic.group..20.categories.) |> summarise(total=sum(Observation)) |> 
  mutate(proportion=(total/6754)*100)


ethnicity_sex_plymouth_total<-ethnicity_sex_plymouth |> group_by(Sex..2.categories.,Ethnic.group..20.categories.) |> 
  summarise(total=sum(Observation))

ggplot(ethnicity_plymouth_total,aes(x=Ethnic.group..20.categories.,y=total))+geom_bar(stat = "identity")

ggplot(ethnicity_sex_plymouth_total,aes(x=Ethnic.group..20.categories.,y=total,fill=Sex..2.categories.))+
  geom_bar(stat = "identity",position = position_dodge(width = 0.9))

# Order by percentage
ethnicity_plymouth_total$ethnicity_f <- factor(
  ethnicity_plymouth_total$Ethnic.group..20.categories.,
  levels = ethnicity_plymouth_total$Ethnic.group..20.categories.[order(ethnicity_plymouth_total$proportion)]
)

ggplot(ethnicity_plymouth_total, aes(x = proportion, y = ethnicity_f)) +
  geom_segment(aes(x = 0, xend = proportion,
                   y = ethnicity_f, yend = ethnicity_f),
               colour = "grey70",
               linewidth = 1) +
...
ggplot(ethnicity_sex_plymouth_total)+
  geom_linerange(aes(x = Ethnic.group..20.categories., ymin = 0, ymax = total, colour = Sex..2.categories.), 
                 position = position_dodge(width = 1))+
  geom_point(aes(x = Ethnic.group..20.categories., y = total, colour = Sex..2.categories.),
             position = position_dodge(width = 1))+
...
education_plymouth_total<-education_plymouth |> group_by(Highest.level.of.qualification..7.categories.) |> 
  summarise(total=sum(Observation)) |> 
  mutate(proportion=(total/6744)*100)


education_sex_plymouth_total<-education_sex_plymouth |> group_by(Sex..2.categories.,Highest.level.of.qualification..7.categories.) |> 
  summarise(total=sum(Observation))

ggplot(education_plymouth_total,aes(x=Highest.level.of.qualification..7.categories.,y=total))+geom_bar(stat = "identity")

ggplot(education_sex_plymouth_total,aes(x=Highest.level.of.qualification..7.categories.,y=total,fill=Sex..2.categories.))+
  geom_bar(stat = "identity",position = position_dodge(width = 0.9))

# Order by percentage
education_plymouth_total$education_f <- factor(
  education_plymouth_total$Highest.level.of.qualification..7.categories.,
  levels = education_plymouth_total$Highest.level.of.qualification..7.categories.[order(education_plymouth_total$proportion)]
)

ggplot(education_plymouth_total, aes(x = proportion, y = education_f)) +
  geom_segment(aes(x = 0, xend = proportion,
                   y = education_f, yend = education_f),
               colour = "grey70",
               linewidth = 1) +
...
ggplot(education_sex_plymouth_total)+
  geom_linerange(aes(x = Highest.level.of.qualification..7.categories., ymin = 0, ymax = total, colour = Sex..2.categories.), 
                 position = position_dodge(width = 1))+
  geom_point(aes(x = Highest.level.of.qualification..7.categories., y = total, colour = Sex..2.categories.),
             position = position_dodge(width = 1))+
...
disability_plymouth_total<-disability_plymouth |> group_by(Disability..3.categories.) |> 
  summarise(total=sum(Observation)) |> 
  mutate(proportion=(total/6746)*100)


disability_sex_plymouth_total<-disability_sex_plymouth |> group_by(Sex..2.categories.,Disability..3.categories.) |> 
  summarise(total=sum(Observation))

ggplot(disability_plymouth_total,aes(x=Disability..3.categories.,y=total))+geom_bar(stat = "identity")

ggplot(disability_sex_plymouth_total,aes(x=Disability..3.categories.,y=total,fill=Sex..2.categories.))+
  geom_bar(stat = "identity",position = position_dodge(width = 0.9))

# Order by percentage
disability_plymouth_total$disability_f <- factor(
  disability_plymouth_total$Disability..3.categories.,
  levels = disability_plymouth_total$Disability..3.categories.[order(disability_plymouth_total$proportion)]
)

ggplot(disability_plymouth_total, aes(x = proportion, y = disability_f)) +
  geom_segment(aes(x = 0, xend = proportion,
                   y = disability_f, yend = disability_f),
               colour = "grey70",
               linewidth = 1) +
...
ggplot(disability_sex_plymouth_total)+
  geom_linerange(aes(x = Disability..3.categories., ymin = 0, ymax = total, colour = Sex..2.categories.), 
                 position = position_dodge(width = 1))+
  geom_point(aes(x = Disability..3.categories., y = total, colour = Sex..2.categories.),
             position = position_dodge(width = 1))+
...
set.seed(2)
age_50_64<-sample(50:64,212, replace = TRUE)
age_35_49<-sample(35:49,174,replace = TRUE)
age_25_34<-sample(25:34,188,replace = TRUE)
age_65_over<-sample(65:90,190,replace = TRUE)
age_15_less<-sample(1:15,117,replace = TRUE)
age_16_24<-sample(16:24,120,replace = TRUE)
